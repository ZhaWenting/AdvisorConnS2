...
import alphabet.adviserconn.InvitationFromDarkNet.adapter.VirusChatAdapter;
...
public class VirusChatActivity extends BaseActivity implements View.OnTouchListener, VirusChatAdapter.ClickTextListener,VirusChatAdapter.LinkedTextListener {
    ...
    protected void initView() {
        ...
        //auto load
        autoSavedList = systemParamsS2Virus.getDataListSeason2("autoSavedListS2");
        mainTxtCount = systemParamsS2Virus.getInt("autoSavedMainCountS2", 0);
        savePosition = systemParamsS2Virus.getInt("autoSavedPositionS2", 0);
        String chapter = systemParamsS2Virus.getString("autoSavedChapter", "");

        if (autoSavedList != null && autoSavedList.size() > 0 && savePosition > 0) {
            if ("".equals(chapter))
                saveChapter = R.raw.season2_day2_virus;
            else
            ...
            // load history
            if (chatAdapter == null) {
                chatAdapter = new VirusChatAdapter(VirusChatActivity.this, currentList, VirusChatActivity.this, VirusChatActivity.this, this);
...
//            LogUtil.d(TAG, "loadFirst->savePosition"+savePosition+" mainTxtCount"+mainTxtCount+" saveChapter"+saveChapter);
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.avatar_ll:
                new ChooseAvatarDialog(this,"VirusChatActivity").show();
                break;
            case R.id.config:
                ConfigDialogS2 configDialogS2 = new ConfigDialogS2(this, delayTime, isEnding,true) {
                    @Override
                    public void restart() {
                        showRestartDialog();
                    }
                    ...
                    @Override
                    public void endings() {
                        new EndingsDialogS2(VirusChatActivity.this,systemParamsS2).show();
                    }

                    @Override
                    public void staff() {
                        new StaffDialogS2(VirusChatActivity.this).show();
                    }
    ...
    private void loadFirst() {
        ...
        currentBean.setContent(TimeUtils.addDay(time, 1));
        currentList = new ArrayList<>();
        currentList.add(currentBean);
        if (saveList == null)
            saveList = new ArrayList<>();
        saveList.add(currentBean);
        chatAdapter = new VirusChatAdapter(VirusChatActivity.this, currentList, this, this, this);
        recyclerview.setAdapter(chatAdapter);
...
//                                    LogUtil.d(TAG, "linked" + currentBean.getHero_text_linked() + " word" + currentBean.getLinked_word());
                                ...
                                case "player_select":
                                    if (lastBean != currentBean) {
                                        ...
                                        tagsDialog = new TagsDialog(VirusChatActivity.this, currentBean.getPlayer_select(), new TagsDialog.TagSelection() {
                                            @Override
...
//                                        tagsDialog.show();
                                ...
                                case "next_day":
                                    ...
                                    currentList = mainList;
                                    systemParamsS2Virus.setString("autoSavedChapter", next_day);
//                                        saveDisabled = false;
                                    break;
...
//                                        sendBtSeason2.setEnabled(true);
                                ...
                                case "wait_flag":
                                    if (lastBean != currentBean) {
                                        ...
                                        systemParamsS2Virus.setBoolean(currentBean.getWait_flag(), true);
                                        mainTxtCount++;
                                        systemParamsS2Virus.setInt("wait_count", mainTxtCount);
                                        lastBean = currentBean;
                                    }
                                    break;

                                case "restart_flag":
                                    if (systemParamsS2Virus.getBoolean(currentBean.getRestart_flag(), false)) {
                                        mainTxtCount = systemParamsS2Virus.getInt("wait_count");
                                        mainTxtCount++;
                                    }
                                    break;
                                ...
                                case "player_select_multi":
                                    ...
                                    if (lastBean != currentBean) {
                                        ...
                                        sendBtSeason2.setEnabled(false);
                                        sendTextSeason2.setEnabled(true);
                                        tagsDialog = new TagsDialog(VirusChatActivity.this, currentBean.getPlayer_select_multi(), new TagsDialog.TagSelection() {
                                            @Override
                                            public void selectTag(int position) {
                                                selection = position;
                                                sendTextSeason2.setText(currentBean.getPlayer_detail().get(position));
...
//                                        LogUtil.d(TAG, "hero_answer_multi->mainTxtCount:" + mainTxtCount);
                                ...
                                //显示弹出的提示
                                case "show_toast":
                                    infoDialog = new InfoDialog(VirusChatActivity.this, currentBean.getShow_toast(), ConvertString.convert(currentBean.getToast_pic()));
                                    infoDialog.show();
                                    mainTxtCount++;
                                    break;
                                case "date":
                                    Integer date = Integer.parseInt(currentBean.getDate());
                                    String time = TimeUtils.addDay(systemParamsS2Virus.getLong("time"), date - 1);
    ...
    private long setIntervalStart() {
        ...
        if (lastBean != currentBean) {
            ...
            systemParamsS2Virus.setLong("startTime", startTime);
            lastBean = currentBean;
        } else
            startTime = systemParamsS2Virus.getLong("startTime");
        return startTime;
    }
    ...
    public void clickText(int selectNum) {
        ...
        //Load the progress
        saveChapter = systemParamsS2Virus.getInt(selectNum + "-saveChapter", 0);
        mainTxtCount = systemParamsS2Virus.getInt(selectNum + "-saveMainCount", 0);
        savePosition = systemParamsS2Virus.getInt(selectNum + "-savePosition", 0);
        saveList = systemParamsS2Virus.getDataListSeason2(selectNum + "-saveList");
...
//                " mainTxtCount:" + mainTxtCount + " count:" + count + " branchTxtCount:" + branchTxtCount);
    ...
    private void saveProgress(int selectNum) {
        systemParamsS2Virus.setInt(selectNum + "-saveChapter", saveChapter);
        systemParamsS2Virus.setDataListSeason2(selectNum + "-saveList", saveList);
        systemParamsS2Virus.setInt(selectNum + "-saveMainCount", mainTxtCount);
        systemParamsS2Virus.setInt(selectNum + "-savePosition", position);
    ...
    private void autoSave() {
        systemParamsS2Virus.setInt("saveChapterS2", saveChapter);
        systemParamsS2Virus.setDataListSeason2("autoSavedListS2", saveList);
        systemParamsS2Virus.setInt("autoSavedMainCountS2", mainTxtCount);
        systemParamsS2Virus.setInt("autoSavedPositionS2", position);
...
//                " mainTxtCount:" + mainTxtCount + " count:" + count + " branchTxtCount:" + branchTxtCount);
    ...
    private void showRestartDialog() {
        new MyAlertDialog(VirusChatActivity.this, "重新开始将会删除所有对话", "确定", "取消") {
            @Override
...
//                    startDialogS2.show();
    ...
    private void restart() {
        ...
        systemParamsS2Virus.setInt("saveChapterS2", R.raw.season2_day2_virus);
        systemParamsS2Virus.setDataListSeason2("autoSavedListS2", null);
        systemParamsS2Virus.setInt("autoSavedMainCountS2", 0);
        systemParamsS2Virus.setInt("autoSavedPositionS2", 0);
        loadFirst();
        updateTimer();
        recyclerview.scrollToPosition(position);
    }
...
//        }
    ...
    public int getLayoutResID() {
        return R.layout.activity_virus_chat;
    }
}
