 ...
 * @date 2015年11月4日
 */
public class SystemParamsS2 {

	private static SystemParamsS2 instance;
	private static SharedPreferences sharedPrederences = null;

	private SystemParamsS2() {
	}

	//在Application初始化
	public static void init(Context context) {
		sharedPrederences = context.getSharedPreferences(Contants.SAVES2, Context.MODE_PRIVATE);
	}
	public static SystemParamsS2 getInstance() {
		
		if(instance == null) {
			synchronized (SystemParamsS2.class) {
				if(instance == null) {
					 instance = new SystemParamsS2();
				}
			}
		}
		return instance;
